...
export async function PATCH(
	...
	try {
		...
		try {
			await session.withTransaction(async () => {
				const txn = await Transaction.findOneAndUpdate(
					{ _id: id, status: "pending", type: "security_deposit" },
					{
						$set: {
							status: newStatus,
							...(note ? { notes: note } : {}),
				...
				if (!txn) {
					// Either not found, not pending, or not a security_deposit
		...
		return NextResponse.json({
			success: true,
			message: `Security deposit ${newStatus} successfully`,
		...
		console.error("[PATCH /api/admin/security-deposits/[id]]", error);
		return NextResponse.json(
