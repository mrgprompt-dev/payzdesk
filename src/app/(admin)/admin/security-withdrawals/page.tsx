...
export default function SecurityWithdrawalsPage() {
	...
	const { data, isLoading, isError, error } = useQuery<TransactionsResponse>({
		queryKey: ["admin-security-withdrawals", page, search, activeStatus],
		queryFn: async () => {
			const res = await axios.get("/api/admin/security-withdrawals", {
	...
	const mutation = useMutation({
		mutationFn: async ({ id, action, note }: { id: string; action: string; note?: string }) => {
			const res = await axios.patch(`/api/admin/security-withdrawals/${id}`, { action, note });
			return res.data;
		},
		onSuccess: () => {
		...
		onError: (err: any) => {
			alert(err?.response?.data?.message || "Failed to process security withdrawal");
		},
	});

	const handleApprove = (id: string) => {
		if (confirm("Are you sure you want to approve this security withdrawal?")) {
	...
	const handleRejectSubmit = (id: string) => {
		...
		if (confirm("Are you sure you want to reject this security withdrawal?")) {
	...
	return (
		<div className="space-y-6">
			<div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
				<h1 className="text-2xl font-semibold text-foreground">Security Withdrawals</h1>
			...
			<Card className="p-4 sm:p-6 shadow-none">
				...
				{isLoading ? (
					...
					<div className="text-destructive py-4 text-center">
						Error loading security withdrawals. {(error as any)?.message}
					</div>
				) : (
